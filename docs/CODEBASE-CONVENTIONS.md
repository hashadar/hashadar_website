...
- **Interiors:** About, Portfolio, Blog, Labs index inherit Home’s surface (type, cream/air, green), not Claim → Statement → Proof. Lock: `docs/research/interior-pages.md`.
...
- **Stack:** Framer Motion for DOM motion site-wide. Home first-fold is the **Claim lockup** (two-line `hasha` / `dar`, Roles as questions, landing line) over a Loop — a treated still with analog VFX. Not name and title over the Home Photo. Public interiors use `PageIntro` (large type + lede), not a second Claim. `three` / R3F may remain as parked dependencies from earlier phases; they must **not** be imported on `/`, about, portfolio, blog, footer, or Labs. Home lock: `docs/research/home-narrative.md`. Interiors: `docs/research/interior-pages.md`. Prior motion inventory: `docs/research/motion-3d-audit.md`.
...
- **Decorative atmospheres:** `SectionBackground` variants are `marketing` (quiet grid/gradient rail), `photography` (minimal — imagery leads), and `none`. Public interiors use `none` or omit `SectionBackground` — no geometric-pattern grids on About, Portfolio, Blog, or the Labs catalogue. `FooterBackground` is static, non-looping accents only. Atmospheres must respect `prefers-reduced-motion` — no infinite decorative loops. Micro-interactions (card hover, lightbox) may stay outside `MotionReveal` but must still honour reduced motion (e.g. no scale zoom). No Loop treatment on interior Photographs.
- **Labs / Admin / Login:** Quieter surfaces — no WebGL; no `SectionBackground` / `HeroMedia` / `MotionRevealGroup` spectacle. The Labs **index** is a public catalogue in the interior language; Lab **shells** stay utility. Login and Admin stay utility `SitePage` surfaces. Adopt shared tokens only when a touch is trivial.
